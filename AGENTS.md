...
`.claude/tools/` 下的五支 Python 腳本（能力自我模型、跨專案帳本、skill 升級閘、斷點續跑、經驗採礦）也不依賴 Claude Code——任何 agent 都能直接呼叫。斷點的**自動記錄**靠 Claude Code 的 hook；其他 agent 拿不到自動記錄，但 `checkpoint.py step／verify／decide／resume` 照樣能用。
