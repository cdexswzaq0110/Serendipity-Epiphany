...
任務切分、派發與同步全部用**作業系統併發模型**命名。借用既有語彙，是因為我已經知道兩個 thread 同時寫同一塊記憶體會出事——那份直覺不用重新教。

| 單位 | 隔離 | 何時用 |
|---|---|---|
| **Coroutine** | 無（同 context） | 需要一套方法，不需要隔離。**預設就是這個** |
| **Thread** | 獨立 context 視窗，共享工作樹 | 唯讀 fan-out：搜尋、審查、第二意見 |
| **Process** | 完整 | 一個垂直切片、一張重票、規劃→實作的邊界 |
...
**最重要的一個區分：寫入衝突是 Mutex，不是依賴。** 標成 `Blocked by` 會憑空製造一條假的關鍵路徑。
...
| 角色 | 誰 | 做什麼 | 不做什麼 |
|---|---|---|---|
| **Orchestrator** | 主 Process | 規劃、派發、驗證、收斂 | 不做 worker 級的工作 |
| **Advisor** | 最強模型的唯讀 Thread | 只在**承諾邊界**被諮詢 | **從不執行** |
| **Worker** | Thread／Process Pool | 通過驗證的最便宜配置 | 不自我核准、不接下一張票 |
...
| 規則 | 核心 |
|---|---|
| **golden-rules** | 來源優先、可追溯、保護使用者工作、以證據宣告完成、最小變更、**留下領悟** |
...
| 想法太大，連要問什麼都不確定 | `se-wayfind` |
| 要被逼問／想法要被戳破／答案在別人身上 | `se-grill` |
| 不確定值不值得做 | `se-feasibility` |
| 專案詞彙混亂 | `se-context-language` |
| 要寫程式 | `se-lazy-ladder` |
| 卡在 bug | `se-debug` |
| 變更完成要審查 | `se-two-axis-review` |
| 要拆票、派工、管平行 | `se-orchestrate` |
| 外部工具不確定能不能用 | `se-capability-doctor` |
...
| `thread-advisor` | Thread（最強模型） | 承諾邊界：結果矛盾、驗證失敗兩次、計畫要結構性改變 |
...
| `process-worker` | Process | 一張 Ticket 的完整實作 |
...
4. **先爬懶惰階梯再寫程式。** 但絕不對理解、信任邊界、資料遺失、安全、無障礙偷懶。
...
流程在 [.claude/ABLATION.md](.claude/ABLATION.md)，理由在 [docs/lessons/0001](docs/lessons/0001-example-harness-bootstrap.md)。

> PLAYBOOK 與 ABLATION 本來就該被實戰改寫。改完記得寫 Lesson。
