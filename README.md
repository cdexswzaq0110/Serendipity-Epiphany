...
├── PLAYBOOK.md            # 四種執行路徑（A 直接執行／B 規劃／C 探索／D 蒐證）
...
| **Process**    | 完整隔離             | 獨立 Work Item、垂直切片、規劃 → 實作邊界  |
...
如果兩個工作只是不能同時修改同一區域，應使用 `Mutex / Critical Section` 描述，而不是標成 `Blocked by`。
...
| 角色               | 執行位置                  | 責任                              | 不負責                        |
| ---------------- | --------------------- | ------------------------------- | -------------------------- |
| **Orchestrator** | 主 Process             | 任務分解、執行派發、同步、驗證、結果整合            | 不執行 Worker 級實作             |
| **Advisor**      | 高能力唯讀 Thread          | 在關鍵 Decision Boundary 提供分析與第二意見 | **不直接修改工作樹**               |
| **Worker**       | Thread / Process Pool | 執行被派發的 Work Item                | 不自行核准結果、不自行取得下一個 Work Item |
...
| 規則                    | 核心                                                           |
| --------------------- | ------------------------------------------------------------ |
| **golden-rules**      | 來源優先、可追溯、保護使用者工作、以證據宣告完成、最小變更、保存可重用經驗                        |
...
| 問題空間過大，需要定位下一個可回答問題           | `se-wayfind`           |
| 需要壓力測試假設、找出缺漏或取得外部決策          | `se-grill`             |
| 需要先判斷技術或產品可行性                 | `se-feasibility`       |
| 專案術語、Domain Language 或命名不一致   | `se-context-language`  |
| 進入程式實作                        | `se-lazy-ladder`       |
| Debug、定位 Root Cause           | `se-debug`             |
| 變更完成，需要 Spec / Standards 雙軸審查 | `se-two-axis-review`   |
| **要做任務分解、執行派發與平行協調**          | **`se-orchestrate`**   |
| 外部工具、MCP、CLI 或環境能力不確定         | `se-capability-doctor` |
...
| `thread-advisor`              | Thread（高能力模型）     | 結果矛盾、驗證連續失敗、計畫需要結構性調整                                              |
...
| `process-worker`              | Process           | 一個 **Work Item** 的完整實作與驗證                                          |
...
[docs/lessons/0001](docs/lessons/0001-example-harness-bootstrap.md)
