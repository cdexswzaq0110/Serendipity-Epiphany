...
- 完全照票做了，但**破壞專案慣例** → Spec 過，Standards 掛。
...
雙方沒有新證據時**停止來回爭辯**，把爭點與現有證據交回 Orchestrator。**不得叫另一個模型來投票**——投票不是證據。

Orchestrator 中立，不加入投票，只檢查雙方是否根據同一個 Snapshot 與可重現證據作出結論；證據不足的一方必須補證據或修正結論。
