...
description: 機器學習專案端到端流程，六階段五道 Gate——問題定義、資料契約與切分、清洗與特徵、Pipeline 與 baseline、CV/OOF 與調參、封裝上線與監控。核心是先定義、再驗證、最後自動化，且驗證設計錯了任何分數都沒意義。當要做 ML 專案、訓練或改進模型、處理資料切分與 leakage、選 metric、做特徵工程、比較候選模型、或要把模型推上線時使用。
---

# ML Lifecycle — 六階段五道 Gate
...
| 階段 | 主要交付物 | 不能跳過的 Gate |
|---|---|---|
| Discovery | Problem Statement、metric、資料與責任人 | 預測結果能驅動明確決策 |
| Data | Data Contract、split plan、EDA report | 無明顯 schema / leakage 錯誤 |
| Modeling | Pipeline、baseline、OOF、experiment ledger | 可重現且優於簡單 baseline |
| Optimization | Ablation、tuning、ensemble comparison | 改善大於 CV noise，segment 無重大退化 |
| Delivery | 完整 pipeline、Model Card、inference contract | 測試、延遲、安全與 rollback 通過 |
| Operations | 監控儀表、告警、feedback/retrain policy | 能偵測 drift 與效能衰退 |
...
| 資料結構 | split | 禁止 |
|---|---|---|
| i.i.d. regression | KFold | 不要在全資料先 fit preprocessing |
| i.i.d. classification | StratifiedKFold | 保持 class ratio 不代表 group 安全 |
| 同客戶多列 | GroupKFold | 同 group 不可跨 train/valid |
| 未來預測 | TimeSeriesSplit / rolling | **禁止 random shuffle**；特徵只能用過去 |
| 空間外推 | spatial holdout | 隨機切分會嚴重樂觀 |
| 最終驗證 | frozen holdout 或 nested CV | 不可重複偷看再改模型 |
...
- **調參只能微調正確方向，不能修復錯誤的 target 或 leakage。** 先確認 feature set、loss、split 與模型家族。
- **Ensemble 只有在 outer/meta cross-fit 穩定勝出時才採用。** Stacking 的 base predictions 必須是 OOF，stacker 也要外層 cross-fit。**拒絕複雜度也是成熟決策**——simple blend 常更可靠。
...
- **交付 artifact**：完整 pipeline（transformers + estimator + threshold/calibrator）、run summary、OOF、feature manifest、inference contract、Model Card。
