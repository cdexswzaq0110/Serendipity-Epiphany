...
- 要改 code 時的**第一個動作**：系統提示顯示在 main/master 上 → 直接 `git checkout -b <type>/<描述>`。開工時的 git 狀態已經在系統提示裡，不必先另外跑 `git branch`／`git status`；開分支可逆、有預設值，**不要為此停下來問**（`dispatch.md` 第 4 條）。session 跑了一陣子、或要做其他 git 寫操作前，才重新查（第 3 條）。
