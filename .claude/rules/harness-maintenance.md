...
> **只在改動這套配置本身時載入**（讀到 `.claude/`、`docs/lessons/`、`docs/eval/` 等底下的檔案）。
> 平常寫程式時不佔 context——2026-09-24 端到端基準量到，每個導入這套配置的專案每一回合都背著它約 1.4k tokens。
...
- `tools/`：**自我理解與學習的工具**——`capabilities.py`（能力自我模型，從原始碼生成）、`lessons.py`（跨專案帳本）、`promote_skill.py`（候選 skill 升級閘）、`checkpoint.py`（斷點續跑，日誌在 `.git/serendipity/`，不進版控）、`mine.py`（從逐字紀錄挖出重複——學習迴圈的第一步）。
...
6. **常駐面要有證據**：新增任何常駐內容（根目錄 `CLAUDE.md`、`.claude/CLAUDE.md`、沒有 `paths:` 的 `rules/*.md`）時，必須在 [ABLATION.md](../ABLATION.md) 的分類表新增一列並填「失敗證據」。填不出證據的不該常駐，改寫成 skill 按需載入。
