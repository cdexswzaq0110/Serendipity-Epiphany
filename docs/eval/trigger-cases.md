...
| 2026-09-23 | `6b26eaa`＋未提交 v2 | — | — | **作廢**：仍有 3 個評測 session 收到簡報（一個案例先跑了 `git status`、被 `--max-turns` 截斷，對下一個 session 來說是真的中斷），且 `--allowedTools` 沒擋住唯讀工具。E：1/5 |
| 2026-09-23 | `c2addba` **v2（乾淨）** | — | — | **E：2/5**——第一個有效數字。條件：`--tools Skill`、`SE_EVAL=1`、工作樹已 commit；13 個評測 session 收到簡報 0、自己跑的工具 0【已確認：日誌】。正例 **5/6**（E1 2/2、E2 2/2、E3 1/2）；碰撞 **1/4 正確**——無上下文的「繼續下一步」3/4 次載入 `se-resume`。不再調 description：5 條案例沒有保留驗證集；真實使用時有斷點會注入簡報、沒斷點時模型看得到上一回合正常結束，這兩個條件都要多回合評測才量得到 |
| 2026-09-23 | `c2addba` | — | — | **D1 無注入重測：1/2**（與 09-22 相同）。先前那次跑的時候 `recall-lessons.sh` 的提示點名了 `se-acquire`；拿掉注入後結果不變，那個 1/2 不是注入造成的 |
